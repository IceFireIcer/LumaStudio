...
- **一括処理**: 選択した複数写真にプリセット / 縮小 / 形式 / 画質を一括適用（進捗バー・キャンセル対応）
- **エディタの前後比較**: 元画像と編集後の分割比較、境界線をドラッグ可能
- **ライトボックスで並列比較** (`C`): 2枚を並べて比較、`Tab` でマーク対象を切替
- **高速な選別**: `X` で除外して次へ、`U` でフラグ解除、マーク後に自動で次へ（設定でOFF可）
- **除外を非表示**: ツールバーのボタンまたは `H` キー
