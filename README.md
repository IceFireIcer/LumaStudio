...
| 文件名 | 类型 | 说明 |
|--------|------|------|
| `Luma Studio Setup 1.0.3.exe` | 安装版 | 推荐大多数用户使用,自动安装到系统并创建桌面快捷方式 |
| `Luma Studio 1.0.3.exe` | 便携版 | 免安装,解压即用,适合移动设备或临时使用 |
...
### v1.0.3
- 新增完整日志系统(实时显示前后端日志,3秒自动刷新)
- 新增自定义输入模态框(替换浏览器prompt)
- 将默认端口从3000/8765改为7443(避免常见端口冲突)
- 改进Electron环境检测逻辑
- 同步源代码到windows-releases分支

### v1.0.2
- 修复收藏夹按钮点击无响应问题
- 修复Electron便携版前端脚本同步问题
