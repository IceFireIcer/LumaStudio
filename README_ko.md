...
- **일괄 처리**: 선택한 여러 사진에 프리셋 / 축소 / 포맷 / 품질을 한 번에 적용 (진행률 표시·취소 지원)
- **편집기 전후 비교**: 원본과 편집 결과 분할 비교, 경계선 드래그 가능
- **라이트박스 나란히 비교** (`C`): 두 장을 나란히 비교, `Tab`으로 마킹 대상을 전환
- **빠른 선별**: `X` 제외 후 다음으로, `U` 플래그 해제, 마킹 후 자동으로 다음 장 (설정에서 끌 수 있음)
- **제외 숨기기**: 툴바 버튼 또는 `H` 키
